    ...
    */
    ...
    else /* domain format, need to do DNS resolution */
    {
        ...
        while(1)
        {
            ...
            if (addrp)
            {
                ...
                {
                    snprintf (resolved_ipv6, sizeof(resolved_ipv6), "%s", ipv6_str);
                }
    ...
    if (sd->rtmod == WAN_RTMOD_IPV6)
    {
        ...
        system ("/usr/bin/service_dhcp dhcp_server-restart" "; "
                "/etc/utopia/service.d/service_mldproxy.sh mldproxy-restart");
#endif
    }
    else
    {
...
#if defined(_LG_OFW_)
        system ("/usr/bin/service_dhcp dhcp_server-stop" "; "
