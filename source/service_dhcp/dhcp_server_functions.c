		...
		if (0 != l_cToken[0])
		{
			...
			if (!strncmp(l_cDhcpEnabled, "TRUE", 4))
			{
				...
				if (!strncmp(l_cIpv4InstStatus, "up", 2))
				{
					...
					sysevent_get(g_iSyseventfd, g_tSysevent_token, "wan-status", wanevent, sizeof(wanevent));
	...
	if (NULL != l_fResolv_Conf)
	{
        ...
        if(Flag==1)
        {
            ...
            if (NULL != l_fResolv_Conf)
            {
                ...
                //fprintf(stderr, "strlen of lan_ipaddr:%d\n",strlen(lan_ipaddr));
...
void get_dhcp_option_for_brlan0( char *pDhcpNs_OptionString )
{
	...
	char l_cLine[255] = {0};
		...
		if ( !(l_cWan_Dhcp_Dns[0]) ) {
			...
			{
				fprintf(stderr, "DHCP SERVER : get_dhcp_option_for_brlan0 :opening file %s failed\n", RESOLV_CONF );
			}
		}
		fprintf(stderr, "DHCP SERVER : get_dhcp_option_for_brlan0:%s\n", l_cWan_Dhcp_Dns );
