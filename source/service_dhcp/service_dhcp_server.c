...
int dnsmasq_server_start()
{
    ...
    if (!strncasecmp(g_cXdns_Enabled, "true", 4)) //If XDNS is ENABLED
    {
        ...
        if (!strncmp(l_cXdnsRefacCodeEnable, "1", 1)){
                sprintf(l_cSystemCmd, "%s -q --clear-on-reload --bind-dynamic --add-mac --add-cpe-id=abcdefgh --dhcp-authoritative -P 4096 -C %s %s --xdns-refac-code",
                                SERVER, DHCP_CONF,dnsOption);
        }else{
                sprintf(l_cSystemCmd, "%s -q --clear-on-reload --bind-dynamic --add-mac --add-cpe-id=abcdefgh --dhcp-authoritative -P 4096 -C %s %s",
                                SERVER, DHCP_CONF,dnsOption);
        }
    }
...
#endif
    {
        sprintf(l_cSystemCmd, "%s --dhcp-authoritative -P 4096 -C %s %s", SERVER, DHCP_CONF,dnsOption);
    }
