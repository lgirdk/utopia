...
int ep_get_allMembers(PL2Net net, PMember live_members, int numMembers){
    ...
    for (ifToken = strtok_r(iflistbuf, " ", &saveptr1); ifToken != NULL; ifToken = strtok_r(NULL, " ", &saveptr1))
    {
        ...
        sscanf(ifToken, MNET_EP_MEMBER_FORMAT(ifnamebuf, live_members[curNumMembers].interface->type->name, (unsigned char *)&live_members[curNumMembers].bReady, &live_members[curNumMembers].pvid));
