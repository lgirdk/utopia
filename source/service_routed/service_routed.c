...
static int radv_start(struct serv_routed *sr)
{
    ...
    if (lock_fd < 0) {
        fprintf(logfptr, "%s: fail to create zebra lock file\n", __FUNCTION__);
        return -1;
    }
    if (flock(lock_fd, LOCK_EX) < 0) {
        if (close(lock_fd) < 0)
            fprintf(logfptr, "%s: fail to close zebra lock file\n", __FUNCTION__);
        return -1;
    }
    ...
    if (flock(lock_fd, LOCK_UN) < 0)
        fprintf(logfptr, "%s: fail to unlock zebra lock file\n", __FUNCTION__);
    if (close(lock_fd) < 0)
        fprintf(logfptr, "%s: fail to close zebra lock file\n", __FUNCTION__);
...
static void rip_force_restart(struct serv_routed *sr)
{
    ...
    if (daemon_stop(RIPD_PID_FILE, "ripd") != 0) {
        fprintf(logfptr, "%s: ripd_stop error\n", __FUNCTION__);
    }
