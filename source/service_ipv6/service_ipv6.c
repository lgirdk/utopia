...
static int gen_dibbler_conf(struct serv_ipv6 *si6)
{
    ...
    */
    ...
    /*This enables so called inactive mode. When server begins operation and it detects that required interfaces are not ready, error message is printed and server exits. However, if inactive mode is enabled, server sleeps instead and wait for required interfaces to become operational.*/
...
static int serv_ipv6_init(struct serv_ipv6 *si6)
{
    ...
    if (syscfg_init() != 0) {
        fprintf(fp_v6_dbg, "%s: fail to init syscfg\n", __FUNCTION__);
        return -1;
    }
#ifdef MULTILAN_FEATURE
