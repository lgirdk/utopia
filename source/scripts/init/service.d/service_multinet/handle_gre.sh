...
update_bridge_config () {
    ...
    for br in $BRIDGES; do
        ...
        br_snoop_rule_v6="`sysevent setunique v6GeneralPurposeFirewallRule " -A FORWARD -o $br -p udp --dport=546:547 -j NFQUEUE --queue-bypass --queue-num $[$br]"`"
...
#  Sysevent calls
    ...
    if [ "$BOX_TYPE" = "XB6" -a x"NULL" != x${2} ] || [ "$BOX_TYPE" = "MV2PLUS" ] ; then
        read_greInst 
    fi
    if [ $recover != "true" ] ; then                            
        check_ssids 1                                       
    ...
    # TODO make this multi-instance
        ...
        if [ "$BOX_TYPE" != "XB6" -o "$MODEL_NUM" = "TG4482A" ] && [ "$BOX_TYPE" != "TCCBR" ] && [ "$BOX_TYPE" != "MV2PLUS" ]; then
           check_ssids
        fi
      fi
    ;;
