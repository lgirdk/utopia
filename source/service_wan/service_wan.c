        ...
        if (sw->rtmod == WAN_RTMOD_IPV6 || sw->rtmod == WAN_RTMOD_DS)
        {
            switch (sw->prot)
            {
                case WAN_PROT_DHCP:
                    ...
                    fprintf(fp_wan_dbg, "%s  Calling sysevent dhcpv6_client-start , RDKB_WAN_MANAGER Enabled\n",__func__);
    ...
    if (sw->rtmod == WAN_RTMOD_IPV6 || sw->rtmod == WAN_RTMOD_DS) {
       if (sw->prot == WAN_PROT_DHCP) {
               fprintf(fp_wan_dbg, "Disabling DHCPv6 Client\n");
                    ...
                    fprintf(fp_wan_dbg, "%s  Calling sysevent dhcpv6_client-stop , RDKB_WAN_MANAGER Enabled\n",__func__); 
#elif defined(CORE_NET_LIB)
