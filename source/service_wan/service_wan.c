...
*/
  ...
  // In ethwan mode send dhcp options part of dhcp-client to get the eMTA dhcp options
	...
	if (strcmp(cEthWanMode, "true") == 0 ) 
         ...
         err = vsystem("/sbin/udhcpc -b -i %s -p %s -V eRouter1.0 -O ntpsrv -O timezone -O 125 -O 4 -O 7 -O 54 -O 100 -O 122 -x %s -s /etc/udhcpc.script", sw->ifname, DHCPC_PID_FILE, options);
    #else
     {
        err = vsystem("/sbin/udhcpc -b -i %s -p %s -V eRouter1.0 -O ntpsrv -O timezone -O 125 -O 4 -O 7 -O 54 -O 100 -O 122 -x %s -s /etc/udhcpc.script", sw->ifname, DHCPC_PID_FILE, options);
     }
...
static int serv_wan_init(struct serv_wan *sw, const char *ifname, const char *prot)
{
    ...
    if (syscfg_init() != 0) {
        fprintf(fp_wan_dbg, "%s: fail to init syscfg\n", __FUNCTION__);
        return -1;
    }
