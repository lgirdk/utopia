...
int Utopia_SetDnsServerByIndex(UtopiaContext *ctx, unsigned long ulIndex, const dns_server_t *dns)
{
    ...
    snprintf(tokenbuf, sizeof(tokenbuf), "dns_%d", index);
    Utopia_SetIndexed(ctx, UtopiaValue_Dns_StaticServer, index, tokenbuf);
...
int Utopia_SetDnsForwardByIndex(UtopiaContext *ctx, unsigned long ulIndex, const relay_forward_t *forward)
{
    ...
    snprintf(tokenbuf, sizeof(tokenbuf), "dns_forward_%d", index);
    Utopia_SetIndexed(ctx, UtopiaValue_Dns_Forward, index, tokenbuf);
